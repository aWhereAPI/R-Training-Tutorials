  ...
  for (x in 1:length(subarea_select)) {
    ...
    admin0_stats <- 
      ...
      dplyr::summarise(tot_POP = sum(population_2020),          
                       ...
                       avg_LTAvgMxT = mean(LTAvgMxT), # stats for LTN Max temp
                       max_LTAvgMxT = max(LTAvgMxT),
                       sd_LTAvgMxT = sd(LTAvgMxT),
                       avg_LTAvgMnT = mean(LTAvgMnT), # stats for LTN Min temp
    ...
    # key on the table is a single column of which precip bin
