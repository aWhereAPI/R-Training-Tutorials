...
repository of R code).  Note that this code has been made robust by checking for
each package's depencies recursivly and then installing one by one.  While this
behavior should not be necessary it has been observed that R will sometimes fail
on installs of packages where a number of depencies need to first be installed
        ...
        while(currentAttemptNumber <= maxInstallAttempts) {
          ...
          tryCatch({
            ...
            if (currentAttemptNumber == maxInstallAttempts) {
              stop(cat('Problem with installing ',dependenciesToInstall.packages.CRAN[y],' which is a necessary dependency 
  ...
  for (x in 1:length(new.packages.CRAN)) {
    ...
    while(currentAttemptNumber <= maxInstallAttempts) {
      ...
      tryCatch({
        ...
        if (currentAttemptNumber == maxInstallAttempts) {
        stop(cat('Problem with installing ',new.packages.CRAN[x],' which is a necessary dependency 
                  \nPlease followup manually'))
        }
      })
    }
